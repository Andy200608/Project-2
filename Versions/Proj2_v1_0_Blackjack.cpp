/*
 * File:    Proj2_Blackjack.cpp
 * Author:  Siyi Zhang
 * Purpose: Revise Project 1 Blackjack with functions, arrays,
...
// SORTING 要同步移动所有 parallel arrays
void swappl(string nam[], float bank[], int wins[], int loss[],
            int push[], int one, int two);
void bubsrt(string nam[], float bank[], int wins[], int loss[],
            int push[], int cnt);
void selsrt(string nam[], float bank[], int wins[], int loss[],
            int push[], int cnt);
void leader(string nam[], float bank[], int wins[], int loss[],
            int push[], int cnt);
...
// 3. Repeat menu，处理 play、stats、sort、search 和 rules。
// 4. Save 五组 matching arrays before exit。
// 5. Sort 会改顺序，所以要 re-search 当前玩家。
...
bool option(int pick, string nam[], float bank[], int wins[],
    ...
    switch (pick) {
        ...
        case 2:
            ...
            return true;
        case 3:
            leader(nam, bank, wins, loss, push, cnt);
            return true;
        case 4:
...
// LINEAR SEARCH:
// 从 index 0 一个个 compare 名字。
// found 后停止推进，没找到 return -1。
// 这里 search 的是玩家，不是 cards。
// 玩家使用 one-word name，所以可以直接 compare string。
int findpl(const string nam[], int cnt, string key) {
    int pos = -1;
    int i = 0;
    while (i < cnt && pos == -1) {
...
// 新玩家从同一个空 index 开始
// cnt 用 pass by reference 送回 main
// 五个 matching arrays 都会初始化。
int addpl(string nam[], float bank[], int wins[], int loss[],
          int push[], int &cnt, int max, string who) {
    int pos = -1;
    if (cnt < max) {
...
// PARALLEL SWAP:
// 一个玩家的 name 和四项记录必须一起移动。
// 两种 sort 都会 call 这个 function。
// one 和 two 是要互换的两个 indexes。
void swappl(string nam[], float bank[], int wins[], int loss[],
            int push[], int one, int two) {
    string word = nam[one];
    nam[one] = nam[two];
...
// BUBBLE SORT:
// compare 相邻 bankroll，从高到低排列。
// 每次 swap 都移动整组 parallel arrays。
// 外层每跑一轮，就完成末端一个位置。
void bubsrt(string nam[], float bank[], int wins[], int loss[],
            int push[], int cnt) {
    for (int end = cnt - 1; end > 0; end--) {
        for (int i = 0; i < end; i++) {
            if (bank[i] < bank[i + 1]) {
                swappl(nam, bank, wins, loss, push, i, i + 1);
            }
        }
    }
}
// SELECTION SORT:
// 在没排好的区间找 wins 最大值。
// 一轮最多 swap 一次，最后仍从高到低。
// best 保存目前看到的最大值 index。
void selsrt(string nam[], float bank[], int wins[], int loss[],
            int push[], int cnt) {
    for (int start = 0; start < cnt - 1; start++) {
        ...
        if (best != start) {
            swappl(nam, bank, wins, loss, push, start, best);
        }
    }
}
// LEADERBOARDS:
// 先用 Bubble Sort 显示 bankroll。
// 再用 Selection Sort 显示 wins。
// setw 让两个 table 保持 formatted。
// main 会在排序以后重新 search 当前玩家。
void leader(string nam[], float bank[], int wins[], int loss[],
            int push[], int cnt) {
    bubsrt(nam, bank, wins, loss, push, cnt);
    cout << "\nBANKROLL - BUBBLE SORT\n";
    cout << left << setw(5) << "Rank" << setw(15)
         << "Player" << right << "Bankroll\n";
    for (int i = 0; i < cnt; i++) {
        cout << left << setw(5) << i + 1 << setw(15)
             << nam[i] << right << '$' << bank[i] << '\n';
    }
    selsrt(nam, bank, wins, loss, push, cnt);
    cout << "\nWINS - SELECTION SORT\n";
    cout << left << setw(5) << "Rank" << setw(15)
         << "Player" << right << "Wins\n";
    for (int i = 0; i < cnt; i++) {
